        ...
        if self.photo_count > 0:
            ...
            try:
                if self.owner.isLocal():#该用户的数据是否位于本地站点
                    ...
                    photo = (Photo.objects.using('remote').filter(owner=self.owner, album=self))[0]           
            except:
                ...
                if self.owner.isLocal():#该用户的数据是否位于本地站点
                    ...
                    self.save(using='remote')#保存                    
                try:
                    if self.owner.isLocal():#该用户的数据是否位于本地站点
                        photo = Photo.objects.get(owner=self.owner, album=self, id=self.cover_photo_id)
                    else:
                        photo = Photo.objects.using('remote').get(owner=self.owner, album=self, id=self.cover_photo_id)                        
