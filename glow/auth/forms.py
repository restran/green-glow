...
class AlbumCreationForm(forms.ModelForm):
    ...
    def clean_topic(self):
        ...
        try:
            if self.user.isLocal():
                Album.objects.get(owner=self.user, topic=topic)
            else:
                Album.objects.using('remote').get(owner=self.user, topic=topic)
...
class AlbumInfoEditForm(forms.ModelForm):
    ...
    def clean_topic(self):
        ...
        try:
            if self.album.owner.isLocal():
                album2 = Album.objects.get(owner=self.album.owner, topic=topic)
            else:
                album2 = Album.objects.using('remote').get(owner=self.album.owner, topic=topic)
        except Album.DoesNotExist:
            return topic
        else:
